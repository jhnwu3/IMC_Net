...
def gen_traindata():
    data = np.load("./data/reaction.npz")
...
def pde(x, y):
    ...
    eq_a = dca_t - 1e-3 * D * dca_xx + kf * ca * cb 
    eq_b = dcb_t - kp
    return [eq_a, eq_b]
...
print(dde.backend.is_gpu_available())
...
model.compile("adam", lr=0.001, external_trainable_variables=[kf, D, kp])
variable = dde.callbacks.VariableValue([kf, D, kp], period=1000, filename="variables.dat")
losshistory, train_state = model.train(iterations=2000, callbacks=[variable])
